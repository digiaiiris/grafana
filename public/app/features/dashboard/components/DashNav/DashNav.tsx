...
export const DashNav = React.memo<Props>((props) => {
  const [allMaintenances, setAllMaintenances] = useState<any>(false);
  ...
  const [hosts, setHosts] = useState<any[]>(false);
  const [selectedMaintenance, setSelectedMaintenance] = useState<any>(false);
  const [confirmIsVisible, setConfirmIsVisible] = useState<boolean>(false);
  const [confirmText, setConfirmText] = useState<string>(false);
  const [confirmAction, setConfirmAction] = useState<any>(false);
  const [ongoingMaintenanceIds, setOngoingMaintenanceIds] = useState<string[]>(false);
